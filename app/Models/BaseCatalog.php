...
class BaseCatalog extends Model {
 private const SOFT_DELETE_TABLES = ['roles','usuarios','categorias_materiales','materiales','origenes_cable','marcas_cable','cables','informes_cable'];
 ...
 public function softDelete(string $table,int $id): bool { $estado=$table==='informes_cable' ? ', estado_informe=\'anulado\'' : ', estado=\'inactivo\''; return $this->execSql("UPDATE $table SET deleted_at=NOW()$estado WHERE id=?",[$id]); }
